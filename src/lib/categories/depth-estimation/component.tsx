...
export const DepthEstimationComponent: FC<Props> = ({
  ...
  const onSubmit = async () => {
    setBusy(true);
    await sdkRef.current?.estimate(attachedImg);
  };
  ...
  return (
    ...
    >
      ...
      <UI.Button
        type="button"
        disabled={busy || !attachedImg}
        text="Generate depth image"
        onClick={onSubmit}
        icon={busy ? <UI.Spinner /> : <Scan size={ICON_SIZE} />}
      />
      {result && <img width={150} src={result} alt="Result" />}
    </UI.Flex>
  );
};
